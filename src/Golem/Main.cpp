...
public:
    ...
    void Main::process( void )
    {
        ...
        if ( Cpl::System::ElapsedTime::expiredMilliseconds( m_timerMarker, m_frameP->getBitTime(), now )
        {
            m_timerMarker = now;
        ...
        uint8_t                intensity = m_rampP->getIntensity( Cpl::System::ElapsedTime::deltaMilliseconds( m_timerMarker ), m_frameP->getBitTime() );
        m_outputP->write( bitColor, intensity, m_currentBit );
        m_currentBit = nextBit;

        // Check for any updates to my policies
        if ( m_dirty )
        {
            Cpl::System::Mutex::ScopeBlock( m_lock );
            m_dirty   = false;
    ...
    ///////////////////////////////
    void Main::setPolicies( Frame*            framePolicyP,
